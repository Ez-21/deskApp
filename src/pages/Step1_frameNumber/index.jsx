...
export default function () {
  ...
  const pushPicture = () => {
    ...
    draftIds.forEach((item, index) => {
      setVideoFraming({ draftIds: [item] }).then((res) => {
        if (res.code == 200) {
          console.log(res.data);
          // 获取抽帧进度
          console.log(item);
          let timeTravel = setInterval(() => {
            getTaskProgress(item, timeTravel);
          }, 1000);
        }
      });
    });
  ...
  // 获取任务进度
  const getTaskProgress = (val, timeTravel) => {
    getProgress({ frameExtractionTaskId: val }).then((res) => {
  ...
  const createPicture = () => {
    ...
    // } else {
      ...
      goPage("/Step2PushPicture", { state: {draftIds,taskId:params.state?.taskId,} });
    // }
  };
  // 获取列表数据
    ...
    getTaskDetial({ task_id })
      .then((res) => {
        ...
        draftList.forEach((item) => {
          item.checked = true;
          // 转换成相对路径
          item.draftImage = item.draftImage.split('\quick')[1].replaceAll('\\','/')
        });
